...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Config.RemoveItemsAfterRPDeath then
		for i=1, #xPlayer.inventory, 1 do
			if xPlayer.inventory[i].count > 0 then
				TriggerEvent("hsn-inventory:server:clearAllItems",identifier) --xPlayer.setInventoryItem(xPlayer.inventory[i].name, 0)
			end
		end
	end
	...
	if Config.RemoveWeaponsAfterRPDeath then
		for i=1, #xPlayer.loadout, 1 do
			TriggerEvent("hsn-inventory:server:clearAllItems",identifier) --xPlayer.removeWeapon(xPlayer.loadout[i].name)
		end
