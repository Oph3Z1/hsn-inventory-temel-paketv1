	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceXPlayer.canCarryItem(itemName, sourceItem.count) then
				TriggerEvent("hsn-inventory:server:removeItem",_source,itemName,amount) --targetXPlayer.removeInventoryItem(itemName, amount)
				TriggerEvent("hsn-inventory:server:addItem",_source,itemName,amount) --sourceXPlayer.addInventoryItem   (itemName, amount)
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			TriggerEvent("hsn-inventory:server:removeItem",source,itemName,count) --xPlayer.removeInventoryItem(itemName, count)
			TriggerEvent("hsn-inventory:server:addItem",source,itemname,count) --inventory.addItem(itemName, count)
...
ESX.RegisterServerCallback('esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	if removeWeapon then
		TriggerEvent("hsn-inventory:server:removeItem",src,weaponName,1) --xPlayer.removeWeapon(weaponName)
	end
